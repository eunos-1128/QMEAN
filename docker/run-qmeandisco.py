...
def _ParseArgs():
    ...
    parser.add_argument("-o", "--output",
                        help="Path to write the JSON data to.")
...
def _TurnIntoQMEANServerJSON(scores_json, ost_ent, seqres, file_nm):
    result_json = {
        ...
        'created':"{:%Y-%m-%dT%H:%M:%S}".format(datetime.now()),
        'models':dict(),
        'method':"QMEANDisCo",
        # This needs to be fixed once we have the data in for doing DisCo
        # sequence searches!
        'smtl_version':"2020-03-04",
    ...
    mdl_for_json = {
        ...
        'scores':scores_json,# scores: create deep copy of entity, let the renumbered copy be equipped with bfactors, run along residues, fetch befactor from renumbered entity and resnum/ chain name from deep copy to undo renumbering
        'chains':dict()
    }
    ...
    for seq in seqres:
        mdl_for_json['chains'][seq.name] = {
            'seqres':str(seq),
            #    'atomseq':seq['atomseq'], no example found yet, no idea if we need to set '-' for gaps
            'name':seq.name
        }
...
def _main():
    ...
    result_json = _TurnIntoQMEANServerJSON(scores_json, mdl_ent, mdl_seq,
    ...
    with open(out_path, 'w') as ofh:
        json.dump(result_json, ofh)#For pretty formatted files: indent=2, sort_keys=True)
